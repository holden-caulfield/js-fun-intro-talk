...
// we get the full event chain back, to do that, we create a few functions to 
// pass _.each or _.map and emit the events
...
function parseFeed(feed) {
  ...
  parser.parseURL(feed.url, {}, function(err, out) {
    if(err) {
      ...
      _.each(_.map(out.items, addSource), emitNews);
    } 
  }); 
}
